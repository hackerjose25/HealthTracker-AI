...
import { ArrowRight, Lock, Mail, X, ShieldCheck, ChevronRight } from 'lucide-react';
...
export default function LoginPage() {
  ...
  // Decode a JWT token (base64) without a library
  ...
  const handleGoogleLogin = useCallback(() => {
    ...
    if (!clientId || clientId === 'your_actual_client_id.apps.googleusercontent.com') {
      // No real Client ID — open Simulation Modal
    ...
    try {
      ...
      client.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          setIsGsiLoading(false);
          setShowSimModal(true); // Fallback if prompt not shown
        }
      });
    } catch (err) {
      setIsGsiLoading(false);
      ...
      <div className="cx" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px' }}>
        <div style={{ maxWidth: 440, width: '100%', padding: 36, background: 'rgba(14, 34, 48, 0.75)', border: '1px solid var(--ink-line)', borderRadius: 20, backdropFilter: 'blur(16px)', boxShadow: '0 24px 64px rgba(0,0,0,0.4)' }}>
          ...
          >
            ...
            {isGsiLoading ? 'Connecting to Google...' : 'Sign in with Google'}
          </button>

          {/* DIVIDER */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            <div style={{ flex: 1, height: 1, background: 'var(--ink-line)' }} />
            <span style={{ fontSize: 11, fontFamily: 'IBM Plex Mono, monospace', color: 'var(--text-muted)', letterSpacing: '0.08em' }}>OR LOGIN WITH EMAIL</span>
            <div style={{ flex: 1, height: 1, background: 'var(--ink-line)' }} />
          </div>
      ...
      {showSimModal && (
        ...
        >
          <div style={{
            ...
            <h3 style={{ fontSize: 19, color: 'var(--text-light)', marginBottom: 8 }}>Sign in with Google</h3>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 6, lineHeight: 1.6 }}>
              No Google Client ID is configured. To enable real Google Sign-In, add your credentials to <code style={{ background: 'rgba(127,231,196,0.1)', padding: '1px 5px', borderRadius: 4, fontSize: 12, color: 'var(--mint)', fontFamily: 'IBM Plex Mono, monospace' }}>.env.local</code>:
            </p>
